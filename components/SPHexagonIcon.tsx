...
// ★ v107.22: static=true ise tüm CSS animation'ları disable edilir.
//   Hexagon donmuş (idle) görünür — küçük chip yerlerde performans + dikkat dağıtmama
//   amacıyla kullanılır (ör: InRoomUserProfile alt aksiyon butonu).
function buildHexagonHTML(p: PaletteSpec, rich: boolean = false, isStatic: boolean = false): string {
...
interface Props {
  ...
  rich?: boolean;
  /** ★ v107.22: true ise tüm animasyonlar disable (sabit hexagon).
   *  Küçük chip yerlerde (örn: InRoomUserProfile alt buton) dikkat dağıtmasın diye kullan. */
  static?: boolean;
}

export default function SPHexagonIcon({ size = 48, style, tier = 'premium', rich = false, static: isStatic = false }: Props) {
  const html = useMemo(() => buildHexagonHTML(PALETTES[tier], rich, isStatic), [tier, rich, isStatic]);
  return (
