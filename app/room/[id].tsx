...
export default function RoomScreen() {
  ...
  const handleAutoMinimize = useCallback(() => {
    ...
    // Deep-link veya restore sonrası stack boş olabilir. Bu durumda router.back()
    // Android activity'yi kapatır; güvenli fallback kullanıcıyı ana sayfaya götürür.
    safeGoBack(router);
  }, [setMinimizedRoom, router, radio]);

  // Android geri sırası: klavye → açık overlay → önceki sayfa. Oda bağlantısı
  // önceki sayfaya dönünce mini karta alınır; yalnızca ana sayfa sistem geri
  // davranışına bırakılır.
  useEffect(() => {
    ...
    const handler = () => {
      ...
      if (
        ...
        showPowerUps || showFollowersSheet || showNotifDrawer
      ) {
        closeAllOverlays();
        return true;
      }
    ...
    showRoomStats, showDonationDrawer, showGiftPanel, showPowerUps, showFollowersSheet,
    showNotifDrawer, closeAllOverlays, handleAutoMinimize,
  ]);
  ...
  const toggleListenerMicRequest = async () => {
    if (!firebaseUser?.uid || !id) return;
    if (myMicRequested) {
      setMyMicRequested(false);
      setMicRequests(prev => prev.filter(u => u !== firebaseUser?.uid));
      try {
        if (isSystemRoom(id as string)) await RoomService.cancelStageRequest(id as string);
      } catch (error: any) {
        setMyMicRequested(true);
        setMicRequests(prev => prev.includes(firebaseUser.uid) ? prev : [...prev, firebaseUser.uid]);
        showToast({ title: 'İstek iptal edilemedi', message: error?.message, type: 'error' });
        return;
      }
      ...
      try {
        if (isSystemRoom(id as string)) await RoomService.requestToSpeak(id as string, firebaseUser.uid);
      } catch (error: any) {
        setMyMicRequested(false);
        setMicRequests(prev => prev.filter(u => u !== firebaseUser.uid));
        showToast({ title: 'Sahne isteği gönderilemedi', message: error?.message, type: 'error' });
        return;
      }
      micReqChannelRef.current?.send({
        type: 'broadcast', event: 'mic_request',
  ...
  const rejectMicRequest = async (uid: string) => {
    try {
      await RoomService.rejectStageRequest(id as string, uid);
    } catch (error: any) {
      showToast({ title: 'İstek reddedilemedi', message: error?.message, type: 'error' });
      return;
    }
    setMicRequests(prev => prev.filter(u => u !== uid));
  ...
  const { stageUsers, listenerUsers, spectatorUsers, viewerCount, amIHost, amIModerator, amIGodMaster, canModerate, isGodOrHost, hostUser, amIActingHost, isOriginalHost, isStageDelegate } = useMemo(() => {
    ...
    const _delegateLeaseMs = room?.stage_delegate_lease_until
      ? new Date(room.stage_delegate_lease_until).getTime()
      : 0;
    const _isStageDelegate = room?.is_system_room === true
      ? _myRole === 'speaker'
        && room.stage_delegate_user_id === firebaseUser?.uid
        && _delegateLeaseMs > Date.now()
      : _myRole === 'speaker' && !_hasAuthorityInRoom;
  ...
  const stageLimits = useMemo(() => {
    ...
    const max = room?.is_system_room
      ? Math.min(Math.max(Number(room.max_speakers) || 20, 1), 20)
      : limits.maxSpeakers;
    return { current, max };
  }, [stageUsers, room]);
  ...
  // Sistem odası kuyruğu yalnızca broadcast belleğinde tutulmaz. Yeni seçilen
  // sorumlu veya yeniden bağlanan istemci sunucudaki FIFO kuyruğunu geri yükler.
  useEffect(() => {
    if (!room?.is_system_room || !firebaseUser?.uid) return;
    let cancelled = false;
    RoomService.getSystemStageRequests(room.id).then(userIds => {
      if (!cancelled) setMicRequests(userIds);
    }).catch(() => {});
    return () => { cancelled = true; };
  }, [room?.id, room?.is_system_room, room?.stage_delegate_user_id, firebaseUser?.uid]);
  ...
  // Sistem odasında yalnızca sahnedeki aktif istemciler sorumluluk seçimine
  // katılır. Lease yenileme; force-close, ağ kopması ve arka planda kalma sonrası
  // görevin başka bir aktif konuşmacıya atomik olarak geçmesini sağlar.
  useEffect(() => {
    if (!room?.is_system_room || !firebaseUser?.uid || myParticipant?.role !== 'speaker') return;
    let cancelled = false;
    const heartbeat = async () => {
      try {
        const result = await RoomService.heartbeatSystemStageDelegate(room.id);
        if (cancelled) return;
        setRoom(prev => prev ? {
          ...prev,
          stage_delegate_user_id: result.delegate_user_id,
          stage_delegate_lease_until: result.lease_until,
          stage_delegate_assigned_at:
            prev.stage_delegate_user_id === result.delegate_user_id
              ? prev.stage_delegate_assigned_at
              : new Date().toISOString(),
        } : prev);
      } catch (error) {
        if (__DEV__) console.warn('[SystemStage] delegate heartbeat failed:', error);
      }
    };
    void heartbeat();
    const timer = setInterval(heartbeat, 30_000);
    return () => { cancelled = true; clearInterval(timer); };
  }, [room?.id, room?.is_system_room, myParticipant?.role, firebaseUser?.uid]);
  ...
  const handleClaimStage = useCallback(async () => {
    ...
    try {
      ...
      if (room?.is_system_room) {
        setRoom(prev => prev ? {
          ...prev,
          stage_delegate_user_id: result.delegate_user_id || firebaseUser.uid,
          stage_delegate_lease_until: result.lease_until || null,
          stage_delegate_assigned_at: new Date().toISOString(),
        } : prev);
      }
      setMyMicRequested(false);
      ...
      if (isNotJoined && firebaseUser?.uid && id) {
        try {
          ...
          if (room?.is_system_room) {
            setRoom(prev => prev ? {
              ...prev,
              stage_delegate_user_id: retry.delegate_user_id || firebaseUser.uid,
              stage_delegate_lease_until: retry.lease_until || null,
              stage_delegate_assigned_at: new Date().toISOString(),
            } : prev);
          }
          setMyMicRequested(false);
  ...
  }, [firebaseUser?.uid, id, setParticipants, lk, participants, room?.is_system_room]);

  // Kontrol çubuğundaki aynı düğme, boş sahnede doğrudan caretaker claim;
  // sahne doluyken ise klasik el kaldırma/iptal davranışını çalıştırır.
  const handleStageRequestPress = useCallback(() => {
    if (stageAction === 'direct_join') {
      handleClaimStage();
      return;
    }
    handleMicRequest();
  }, [stageAction, handleClaimStage, handleMicRequest]);
  ...
  const handleGhostSeatPress = useCallback(async () => {
    ...
    if (!isHost && !isMod && isCaretakerMode && stageUsers.length === 0) {
  ...
  }, [room, firebaseUser?.uid, speakingMode, myMicRequested, profile?.display_name, participants, ownerTier, lk, isCaretakerMode, stageUsers.length, handleClaimStage]);
  ...
  return (
    ...
    >
      ...
      <View style={{
        ...
        <SpeakerSection stageUsers={stageUsers}
          stageDelegateUserId={room?.stage_delegate_user_id}
